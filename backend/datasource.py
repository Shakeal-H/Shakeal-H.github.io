import psycopg2
from backend import psqlConfig as config
import numpy as np
...
class DataSource:
    ...
    def __init__(self):
        ...
        variable queried_params, a variable that holds the parameters of our parametrized
        final query to our database based on user specifications
        '''
        self.queried_params = []
        try:
    ...
    def get_additional_quant_stats(self, quant_dict):
        ...
        if not self.is_empty_key(quant_dict, "experience"):
            ...
            queried_columns.append("experience")
        ...
        if not self.is_empty_key(quant_dict, "friendship"):
            ...
            queried_columns.append("friendship")
    ...
    def get_cat_stats(self, cat_dict):
        ...
        if not self.is_empty_key(cat_dict, "species"):
            queried_values.append(self.get_species(cat_dict["species"]))
            queried_columns.append("species")
    ...
    def get_type_effectiveness(self, type_effect_dict):
        ...
        if not self.is_empty_key(type_effect_dict, "against_grass"):
            queried_values.append(self.get_againstGrass(type_effect_dict["against_grass"][0], type_effect_dict["against_grass"][1]))
            queried_columns.append("against_grass")

        if not self.is_empty_key(type_effect_dict, "against_ice"):
            queried_values.append(self.get_againstIce(type_effect_dict["against_ice"][0], type_effect_dict["against_ice"][1]))
        ...
        if not self.is_empty_key(type_effect_dict, "against_ground"):
            queried_values.append(self.get_againstGround(type_effect_dict["against_ground"][0], type_effect_dict["against_ground"][1]))
    ...
    def get_friendship(self, friendship_min, friendship_max):
        ...
        '''
        query = "friendship BETWEEN %s AND %s"
    ...
    def get_experience(self, experience_min, experience_max):
        ...
        '''
        query = "experience BETWEEN %s AND %s"
    ...
    def get_species(self, species_selected):
        '''
        Returns the part of the final query that filters by the species value

        PARAMETERS:
            species_selected - a list of species selected by the user

        RETURN:
            a part of the final query that filters by the species value
        '''
        #use an "=" sign if there is only one value selected, use the IN operator otherwise
        if len(species_selected) == 1:
            query =  "species = %s"
            self.queried_params.append(species_selected[0])
        else:
            query = "species IN %s"
            self.queried_params.append(species_selected)
    ...
    def create_base_stats_radar_plot(self, pokemon_name):
        '''
        Returns an image of a radar plot of the base stats for a named pokemon

        PARAMETERS
            pokemon_name - the name of the pokemon for which we make the radar plot

        RETURN:
            an image of a radar plot of the base stats for a named pokemon
        '''
        query = "SELECT hp, attack, defense, special_attack, special_defense, speed FROM pokemonattributes WHERE name = %s"

        try:
            cursor = self.connection.cursor()
            cursor.execute(query, (pokemon_name.title(),))
        ...
        base_stats = base_stats[0]
        base_stats = list(base_stats)
        attributes = ["hp", "attack", "defense", "special attack", "special defense", "speed"]

        # for i in range(len(base_stats)):
        #     base_stats[i] = attributes[i] + " "+ str(base_stats[i])

        base_stats = tuple(base_stats)

        plt.style.use('ggplot')
        pokemon = [base_stats[0], base_stats[1], base_stats[2], base_stats[3], base_stats[4], base_stats[5]]
...
def main():
    ...
    radar_test = data.create_base_stats_radar_plot("Charizard")
    print(radar_test)
